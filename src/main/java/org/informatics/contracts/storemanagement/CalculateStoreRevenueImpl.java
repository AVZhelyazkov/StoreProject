...
import org.informatics.contracts.storemanagement.contracts.CalculateStoreRevenue;
...
public class CalculateStoreRevenueImpl implements CalculateStoreRevenue {
    ...
    public BigDecimal calculateStoreRevenue() {
        BigDecimal allReceiptsTotalSummed = store.getCheckouts().stream()
                .flatMap(checkout -> checkout.getCachedReceipts().stream())
                .map(Receipt::getReceiptTotal)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal allBaseProductPriceSummed = store.getCheckouts().stream()
                .flatMap(checkout -> checkout.getCachedReceipts().stream())
